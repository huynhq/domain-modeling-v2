...
class MoneyTests: XCTestCase {
    ...
    func testCustomStringConvertible() {
        let result = tenUSD.description
        XCTAssert(result == "USD10.0")
    }
    
    func testPlusOperator() {
        let result = tenUSD + twelveUSD
        XCTAssert(result.description == "USD22.0")
    }
    
    func testSubtractOperator() {
        let result = tenUSD - twelveUSD
        XCTAssert(result.description == "USD-2.0")
    }
    
    func testDoubleExtension() {
        let thirteenUSD = 13.usd
        XCTAssert(thirteenUSD.amount == 13.0)
        XCTAssert(thirteenUSD.currency == "USD")
    }
}
